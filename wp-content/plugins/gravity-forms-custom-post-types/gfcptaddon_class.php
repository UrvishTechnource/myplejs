    ...
    class GFCPTAddonLogic {
      ...
      protected $_included_linked_dropdown_js;
        ...
        public function init() {
            ...
            add_action('gform_after_submission_16', array(&$this, 'save_taxonomies_special'), 10, 2);

            add_action('gform_after_submission_19', array(&$this, 'save_taxonomies_special'), 10, 2);

            add_action('gform_after_submission_13', array(&$this, 'save_taxonomies_special'), 10, 2);

            add_action('gform_after_submission_4', array(&$this, 'save_taxonomies_special'), 10, 2);
...
';
  ...
  <?php
   ...
   /*
         * Loop through all fields and save any linked taxonomies
         */
        function save_taxonomies_special( $entry, $form ) {
            // Check if the submission contains a WordPress post
            if ( isset ( $entry['post_id'] ) ) {
                ...
                foreach( $form['fields'] as &$field ) {
                    ...
                    $this->save_taxonomy_field_special( $field, $entry, $taxonomy );
                }
            }
        }
         ...
         * Save linked taxonomies for a single field
         */
        function save_taxonomy_field_special( &$field, $entry, $taxonomy ) {
          ...
          if ( is_array($this->_taxonomies) && array_key_exists( $field_id, $this->_taxonomies ) ) {
            ...
            if ( $type == 'checkbox' || $type == 'multiselect' ) {
              ...
              $term_ids = $taxonomy_data['term_ids'];
              if ( !empty ( $term_ids )) {
                wp_set_object_terms( $entry['post_id'], $term_ids, $taxonomy, false );
              }
              ...
              if (isset($taxonomy_data['term_to_add'])) {
                ...
                if ( !is_wp_error($term_info) ) {
                  $term_id = (int) $term_info['term_id'];
                  wp_set_object_terms( $entry['post_id'], $term_id, $taxonomy, false );
                }
            ...
            $terms = wp_get_object_terms($entry['post_id'], $taxonomy);
            foreach ($terms as $term) {
            $parenttags = get_ancestors($term->term_id,$taxonomy);
            wp_set_object_terms( $entry['post_id'], $parenttags, $taxonomy, true );
            }
          }
        }
