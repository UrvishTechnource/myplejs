...
<div class="login" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form name="loginform" id="loginform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'login' ); ?>" method="post">
		<p>
			<label for="user_login<?php $template->the_instance(); ?>"><?php _e( 'E-mail', 'theme-my-login' ) ?></label>
...
?>
		...
		<?php $currentlang = get_bloginfo('language'); if($currentlang=="en-US"):?>
				<p class="submit">
					...
					<input type="hidden" name="redirect_to" value="http://myplejs.se/en/my-page/" />
					<input type="hidden" name="testcookie" value="1" />
					<input type="hidden" name="instance" value="<?php $template->the_instance(); ?>" />
				</p>
			<?php else: ?>
				<p class="submit">
					<input type="submit" name="wp-submit" id="wp-submit<?php $template->the_instance(); ?>" value="<?php _e( 'Logga in', 'theme-my-login' ); ?>" />
